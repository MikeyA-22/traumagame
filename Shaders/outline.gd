...
func _init() -> void:
	var shader_file:RDShaderFile = load("res://Shaders/outline.glsl")
	var shader_spirv := shader_file.get_spirv()
	shader = rd.shader_create_from_spirv(shader_spirv)
	pipeline = rd.compute_pipeline_create(shader)
	
	var parameter_data := PackedFloat32Array([0.0, 0.0, 0.0, 0.0]).to_byte_array()
...
##this is where all our processing happens and we do the rendering effect
func _render_callback(effect_callback_type: int, render_data: RenderData) -> void:
	var render_scene_buffer: RenderSceneBuffersRD = render_data.get_render_scene_buffers()
	if !render_scene_buffer:
		return
	...
	var parameter_data := PackedFloat32Array([size.x, size.y, 0.0, 0.0]).to_byte_array()
	...
	color_layer_uniform.add_id(render_scene_buffer.get_color_layer(0))
